...
def save_mht_all_images(input_path):
    # 使用输入文件名作为子目录名
    sub_path_name = make_dir(os.path.splitext(os.path.basename(input_path))[0])
        ...
        for line in f:
            if str(boundary) in str(line):
                if 'image' in content_type:
                    ...
                    # 从Content-Location中提取大括号内的内容作为文件名
                    if '{' in content_location and '}' in content_location:
                        filename = content_location.split('{')[1].split('}')[0]
                    else:
                        # 如果没有大括号，则使用默认文件名
                        filename = 'image_' + str(int(time.time() * 1000))
                ...
                content_location = str(line).split(':')[-1].strip()
            else:
                if b'\r\n' == line and content == b'':
                    pass
                else:
